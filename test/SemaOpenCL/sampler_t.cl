...
void kernel ker(sampler_t argsmp) {
  ...
  const sampler_t const_smp3 = const_smp; // expected-error{{initializer element is not a compile-time constant}}
  const sampler_t const_smp4 = f(); // expected-error{{initializer element is not a compile-time constant}}
  const sampler_t const_smp5 = 1.0f; // expected-error{{initializing '__constant sampler_t' with an expression of incompatible type 'float'}}

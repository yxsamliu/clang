...
CodeGenModule::GetOrCreateLLVMGlobal(StringRef MangledName,
  ...
  auto ExpectedAS = GetGlobalVarAddressSpace(D, LanguageExpected);
  auto ExpectedTargetAS = getContext().getTargetAddressSpace(ExpectedAS);
  assert(ExpectedTargetAS == Ty->getPointerAddressSpace());
  if (AddrSpace != ExpectedAS)
    return getTargetCodeGenInfo().performAddrSpaceCast(GV, AddrSpace,
                                                       ExpectedAS, Ty);
...
unsigned
CodeGenModule::GetGlobalVarAddressSpace(const VarDecl *D,
                                        GetGlobalVarAddressSpaceKind K) {
  assert(K == TargetFavored || K == LanguageExpected);
  ...
  if (isTypeConstant(D->getType(), false)) {
    if (auto ConstAS = getTarget().getConstantAddressSpace())

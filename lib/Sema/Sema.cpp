...
void Sema::setCurrentOpenCLExtensionForDecl(Decl *D) {
  llvm::errs() << "[Sema::setCurrentOpenCLExtensionForDecl]\n";
  llvm::errs() << "Decl: "; D->dump(); llvm::errs() << '\n';
  llvm::errs() << "Curr extension: " << CurrOpenCLExtension << '\n';
  if (CurrOpenCLExtension.empty())
    return;
...
bool Sema::checkOpenCLDisabledTypeDeclSpec(const DeclSpec &DS, QualType QT) {
  ...
  if (auto TagT = dyn_cast<TagType>(QT.getCanonicalType().getTypePtr()))
    Decl = TagT->getDecl();
  llvm::errs() << "[Sema::checkOpenCLDisabledTypeDeclSpec]\n";
  llvm::errs() << "QualType: "; QT.dump(); llvm::errs() << '\n';
  if (Decl) {
    llvm::errs() << "Decl: "; Decl->dump(); llvm::errs() << '\n';
  }
  ...
  if (DL != OpenCLDeclExtMap.end()) {
    for (auto &I : DL->second) {
      if (I != CurrOpenCLExtension && !getOpenCLOptions().isEnabled(I)) {
        Diag(DS.getTypeSpecTypeLoc(), diag::err_type_requires_extension)
            << QT << I;
        return true;
      }
    }
  ...
  auto Loc = OpenCLTypeExtMap.find(QT.getCanonicalType().getTypePtr());
  if (Loc == OpenCLTypeExtMap.end())
    return false;
  bool Disabled = false;
  for (auto &I : Loc->second) {
